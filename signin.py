...
def forget_pass():
    def change_password():
        if user_entry.get()=='' or newpass_entry.get()=='' or confirmpass_entry.get()=='':
            ...
            con=pymysql.Connect(host='localhost',user='root',password='Raj@690',database='userdata')
            mycursor=con.cursor()
    ...
    user_entry=Entry(window,width=25,font=('arial', '11', 'bold'), bg='white', fg='magenta2',bd=0)
    ...
    newpass_entry = Entry(window, width=25, font=('arial', '11', 'bold'), bg='white', fg='magenta2', bd=0)
    ...
    confirmpass_entry = Entry(window, width=25, font=('arial', '11', 'bold'), bg='white', fg='magenta2', bd=0)
    confirmpass_entry.place(x=470, y=320)

    Frame(window, width=250, height=2, bg='orchid1').place(x=470, y=340)

    submitButton=Button(window,text='submit',bd=0,bg='magenta2',fg='white',font=('Opens Sans','16','bold'),width=19,cursor='hand2',
...
def login_user():
    if usernameEntry.get()=='' or passwordEntry.get()=='':
        ...
        try:
            con=pymysql.Connect(host='localhost',user='root',password='690690')
            mycursor=con.cursor()
        except:
...
newaccountButton=Button(login_window,text='Create new account',font=('Open Sans',9,'bold underline'),fg='blue',bg='white'
                   ,activeforeground='blue',activebackground='white',cursor='hand2',bd=0,command=signup_page)

newaccountButton.place(x=723,y=500)
